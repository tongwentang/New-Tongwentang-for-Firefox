...
var TongWen = (function () {
  ...
  function loadSettingData(preferences) {
    // 載入預設的轉換表
    addS2TTable(default_S2T_table);
    addT2STable(default_T2S_table);
    addS2TTable(default_PS2T_table);
    addT2STable(default_PT2S_table);
    ...
    if (preferences.userPhraseEnable) {
      ...
      let newPT2S = {};
      for (let k in preferences.userPhraseSimpList) {
        let nk = convertCharacter(k, flagSimp);
        newPT2S[nk] = preferences.userPhraseSimpList[k];
      }
      addT2STable(newPT2S);

      let newPS2T = {};
      for (let k in preferences.userPhraseTradList) {
        let nk = convertCharacter(k, flagTrad);
        newPS2T[nk] = preferences.userPhraseTradList[k];
      }
      addS2TTable(newPS2T);
    }
    // 強制字型設定
